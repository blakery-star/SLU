...
class SLUBertOnei(nn.Module):

    def __init__(self, config):
        super(SLUBertOnei, self).__init__()
        self.config = config
    ...
    def forward(self, batch):
        ...
        utt = batch.utt
        out_sets = batch.out_sets
        embeds = torch.zeros(B,max(lengths),self.config.embed_size).to(self.device)
      
        for batch_id,out_set in enumerate(out_sets):
            for idx,word in enumerate(out_set):
                if word == UNK:
                    out_set[idx] = '[UNK]'
                if word == PAD:
                    out_set[idx] = '[PAD]'
            ...
            with torch.no_grad():
                embed = self.bert_model(bert_input_pt).last_hidden_state[0]
